...
LexItem id_or_kw(const string &lexeme, int linenum) {
	...
	map<string, Token> kwmap = {
		{"print", PRINT}, {"if", IF},	  {"else", ELSE},		{"string", STRING}, {"int", INT},	  {"float", FLOAT},
		{"char", CHAR},	  {"bool", BOOL}, {"program", PROGRAM}, {"true", TRUE},		{"false", FALSE},
	...
	for (int i = 0; i < lexeme.length(); i++) { strlexeme[i] = tolower(strlexeme[i]); }

	tt = IDENT;
	auto kIt = kwmap.find(strlexeme);
	if (kIt != kwmap.end()) { tt = kIt->second; }

	if (tt == TRUE || tt == FALSE) tt = BCONST;
	...
	while (in.get(ch)) {
		...
		switch (lexstate) {
			...
			case INSTR:
				...
				if (ch == '\"') { return LexItem(SCONST, lexeme, linenum); }
				lexeme += ch;
				break;
